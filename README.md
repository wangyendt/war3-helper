...
- **血条 / 蓝条常显**：自动替你按住 `Alt`。你一操作就松开、停手 0.3 秒自动按回，所以不会误发 `Alt+点击` 信号
...
**打开发现设置变回默认了？**
配置读不出来时程序不会再默默套用默认值——它会把原文件改名保留成
`config.json.broken-<时间>.json` 并弹窗告诉你路径，改回来即可。
诊断配置位置可以编译运行 `tools/DumpConfig.cs`，它会打印程序实际用的是哪个文件。
...
| `src/Engine.cs` | 键鼠底层钩子、改键、组合键喊话、APM、血条常显、商店模式 |
