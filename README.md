...
**TP 期间按 Shift+空格跳刀，为什么会打断 TP？**
空格改成小键盘物品键时，Windows 可能额外生成一次 Shift 松开事件，让游戏收到的
物品指令失去 Shift。助手现在会在发送小键盘键期间拦截这种系统生成的事件，保留排队状态；
纯扫描码模式下，小键盘目标也会保留虚拟键码，避免被译成方向键。

按住 Shift 时也会跳过「物品键先选中英雄」，直接对当前选中单位使用物品，不插入 F1。
先选中英雄，开始 TP，再按住 Shift 使用跳刀并点击落点，点完再松开 Shift。
需开启「按住Ctrl/Alt/Shift时仍改键」。
...
编译测试后，可额外运行 Shift+小键盘的 Windows 输入集成测试：

```powershell
.\obj\Tests.exe --input
```

这项测试需要桌面解锁、NumLock 开启、未按住修饰键且魔兽不在前台。
它通过真实键盘钩子触发空格改键，并拦住测试物品键，检查 Windows 处理后是否仍带 Shift。
...
| `src/MainForm.cs` | 主界面 |
| `tools/Test*.cs` | 版本切换、改键引擎及可选的 Windows 输入集成测试 |
